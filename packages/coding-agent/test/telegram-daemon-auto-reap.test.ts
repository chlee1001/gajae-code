...
describe("issue #4403 auto-reap", () => {
	test("watchdog self-terminates on superseded full tuple", async () => {
		...
		try {
			...
			await fs.promises.writeFile(
				paths.lock,
				JSON.stringify({ pid: oldState.pid, incarnation: oldState.incarnation, ownerId: oldState.ownerId, acquisitionId: oldState.acquisitionId, startedAt: 1 }) + "\n",
			);
			let stopped = false;
			...
			} as unknown as new (opts: { pid: number; settings: Settings; ownerId: string }) => { run(): Promise<void>; requestStop(r: string): void };
			// Use injected readDaemonState to force supersession detection via full tuple
			const readDaemonState = async () => oldState;
			const deps = {
	...
	test("orphan sweep converges 16 stale + 1 current to 1 live", async () => {
		...
		try {
			...
			const pidIncarnation = (pid: number) => (aliveSet.has(pid) ? `linux:${pid}` : pid === currentPid ? currentIncarnation : undefined);
			const processReference = (pid: number) => {
				if (aliveSet.has(pid)) {
					return { incarnation: `linux:${pid}`, termination: "cooperative" as const, signalRoot: () => { aliveSet.delete(pid); } };
				}
				return undefined;
			};
	...
	test("startup sweep refuses current, PID-reused, foreign, similar cmdline", async () => {
		...
		try {
			...
			await writeTelegramOwnerMarker(fsImpl2 as unknown as TelegramDaemonFs, agentDir, {
				version: 1, agentDir, agentDirDigest: digest, ownerId: "cur", acquisitionId: "cur-acq", pid: currentPid, incarnation: currentIncarnation, createdAt: Date.now(), startedAt: Date.now(),
			} as never);
			// PID-reused stale (incarnation mismatch)
			await writeTelegramOwnerMarker(fsImpl2 as unknown as TelegramDaemonFs, agentDir, {
				version: 1, agentDir, agentDirDigest: digest, ownerId: "reused", acquisitionId: "reused-acq", pid: 60000, incarnation: "linux:11111", createdAt: Date.now(), startedAt: Date.now(),
			} as never);
			...
			await writeTelegramOwnerMarker(fsImpl2 as unknown as TelegramDaemonFs, foreignDir, {
				version: 1, agentDir: foreignDir, agentDirDigest: foreignDigest, ownerId: "foreign", acquisitionId: "foreign-acq", pid: 60001, incarnation: "linux:60001", createdAt: Date.now(), startedAt: Date.now(),
			} as never);
	...
	test("malformed/missing state and unavailable incarnation fail closed", async () => {
		...
		try {
			...
			await writeTelegramOwnerMarker(fs.promises as unknown as TelegramDaemonFs, agentDir, {
				version: 1, agentDir, agentDirDigest: (await import("../src/sdk/bus/daemon-paths")).agentDirDigest(agentDir), ownerId: "x", acquisitionId: "x-acq", pid: 70000, incarnation: "linux:70000", createdAt: Date.now(), startedAt: Date.now(),
			} as never);
			const fsImpl = fs.promises as unknown as TelegramDaemonFs;
			const { decisions } = await reapTelegramDaemonOrphans({
				agentDir,
			...
			expect(decisions.some(d => d.kind === "refused" && (d as { reason?: string }).reason === "incarnation_unavailable")).toBe(true);
		} finally {
			fs.rmSync(agentDir, { recursive: true, force: true });
		}
	});
