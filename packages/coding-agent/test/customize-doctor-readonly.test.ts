...
describe("customize doctor read-only contract (#4406)", () => {
	...
	it("fresh empty agent dir gains zero files (JSON mode)", async () => {
		const { agentDir, configRoot, projectDir } = await makeIsolatedEnv("gjc-empty-json");
	...
	it("fresh empty agent dir gains zero files (text mode)", async () => {
		const { agentDir, configRoot, projectDir } = await makeIsolatedEnv("gjc-empty-text");
	...
	it("legacy agent-dir settings.json is untouched (JSON mode)", async () => {
		const { agentDir, configRoot, projectDir } = await makeIsolatedEnv("gjc-legacy-agent-json");
	...
	it("legacy agent-dir settings.json is untouched (text mode)", async () => {
		const { agentDir, configRoot, projectDir } = await makeIsolatedEnv("gjc-legacy-agent-text");
	...
	it("legacy project .gjc/settings.json is untouched", async () => {
		const { agentDir, configRoot, projectDir } = await makeIsolatedEnv("gjc-legacy-project");

		const gjcDir = path.join(projectDir, ".gjc");
		await fs.mkdir(gjcDir, { recursive: true });
		const legacyContent = JSON.stringify(
			{ "gjc.ralplan.maxIterations": 5, model: "legacy-project-model" },
			null,
			2,
		);
	...
	it("existing config.yml and project customization trees retain name/content/mode/mtime", async () => {
		const { agentDir, configRoot, projectDir } = await makeIsolatedEnv("gjc-existing-config");
	...
	it("malformed config.yml reports diagnostics without repair writes", async () => {
		const { agentDir, configRoot, projectDir } = await makeIsolatedEnv("gjc-malformed");
	...
	it("repeated doctor calls are idempotently read-only", async () => {
		const { agentDir, configRoot, projectDir } = await makeIsolatedEnv("gjc-idempotent");
	...
	it("produces valid JSON report with expected schema (JSON mode)", async () => {
		const { agentDir, configRoot, projectDir } = await makeIsolatedEnv("gjc-schema");
	...
	it("redacts secrets from both text and JSON output", async () => {
		const { agentDir, configRoot, projectDir } = await makeIsolatedEnv("gjc-redact");
	...
	it("preserves precedence behavior: project copy of bundled skill is shadowed (#4350)", async () => {
		const { agentDir, configRoot, projectDir } = await makeIsolatedEnv("gjc-precedence");

		await fs.mkdir(path.join(projectDir, ".gjc", "skills"), { recursive: true });
		await fs.writeFile(
			path.join(projectDir, ".gjc", "skills", "ralplan.md"),
		...
		const ralplanItems = skillSurface.items.filter((i: { name: string }) => i.name === "ralplan");
		const nativeCopy = ralplanItems.find((i: { provider: string }) => i.provider === "native");
		expect(nativeCopy).toMatchObject({
			status: "shadowed",
			reason: "shadowed-by-precedence",
		});
		expect(nativeCopy?.precedence?.shadowedBy).toMatchObject({ provider: "bundled" });

		const bundled = ralplanItems.find((i: { provider: string }) => i.provider === "bundled");
	...
	it("reads canonical config.yml values for doctor policy", async () => {
		const { agentDir, configRoot, projectDir } = await makeIsolatedEnv("gjc-config-read");
